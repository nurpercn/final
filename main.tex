...
1 & 1 & Gas Amount Determination & \SI{43}{\degreeCelsius} & Ambient & 1 & 10 \\ \hline
2 & 2  & Pull-down & \SI{43}{\degreeCelsius} & Ambient & All samples & 5 \\ \hline
3 & 3  & Energy Efficiency & \SI{16}{\degreeCelsius} & Ambient & 1 & 15 \\ \hline
3 & 4  & Energy Efficiency & \SI{25}{\degreeCelsius} & Ambient & 1 & 16 \\ \hline
...
\(\omega^V\) & Weight assigned to voltage-capacity deviation in chamber assignment \\ \hline
...
The assignment model uses proportional workload targets rather than absolute horizon-based capacity requirements. In implementation, it is solved over the demanded environment set induced by \(S\), denoted here by \(\mathcal{E}(S)=\{e\in\mathcal{E}:W_e(S)>0\}\). Let \(x_{ce}=1\) if chamber \(c\) is assigned to environment \(e\), and let \(\mathcal{E}^H\) be the set of humidity-controlled environments. Consistent with the implementation, the exact assignment model minimizes weighted squared deviations from workload-based target capacities. The voltage term is weighted by \(\omega^V=2.0\) in the computational study:
\begin{align}
\min\quad &
\sum_{e\in\mathcal{E}(S)}
\left(\sum_{c\in\mathcal{C}}M_cx_{ce}-T_e\right)^2
+\omega^V
\sum_{e\in\mathcal{E}(S)}
\left(\sum_{c\in\mathcal{C}}M_cv_cx_{ce}-T_e^V\right)^2 \\
\text{s.t.}\quad
& \sum_{e\in\mathcal{E}(S)}x_{ce}=1 && \forall c\in\mathcal{C},\\
& \sum_{c\in\mathcal{C}}x_{ce}\ge 1 && \forall e\in\mathcal{E}(S),\\
& \sum_{c\in\mathcal{C}}v_cx_{ce}\ge 1 && \forall e\in\mathcal{E}^V(S),\\
& x_{ce}\le h_c && \forall c\in\mathcal{C},\ e\in\mathcal{E}(S)\cap\mathcal{E}^H,\\
& x_{ce}\in\{0,1\} && \forall c\in\mathcal{C},\ e\in\mathcal{E}(S).
\end{align}
The branch-and-bound implementation solves this model over chamber types with identical station count, humidity capability, and voltage capability to remove symmetry while preserving exactness. The resulting assignment is then refined by swap and move neighborhoods. A candidate is retained only if it preserves environment coverage, humidity feasibility, and voltage feasibility. Feasible candidates are evaluated with the lightweight EDD evaluation mode described in Section~\ref{subsec:scheduling}, and a first-improvement rule is used.
...
\(\omega^V\) & Voltage-deviation weight in chamber assignment & 2.0 \\
