...
The selected proposed configuration, defined by the adaptive \(0.20\)--\(0.30\) VNS policy with \(K_{\max}=200\), was then compared with the principal benchmark variants. Table~\ref{tab:main_benchmark_comparison} reports the results for the fixed-sample policy, the sample-optimization variant without outer-loop reconfiguration, the variant without VNS diversification, and the selected proposed configuration.
