...
The fixed-sample policy performs worst, with an average gap of \(361.93\%\) and a standard deviation of \(198.43\%\). Although this policy is computationally very fast and uses only 150 samples on average, its poor performance indicates that the industrial rule of assigning a fixed number of samples to every product is not robust under heterogeneous planning conditions. The selected proposed configuration uses 380 samples on average, but the improvement should not be interpreted simply as a uniform increase in prototype quantity. Rather, the benefit comes from deciding where additional samples are valuable and coordinating those decisions with chamber reconfiguration and VNS diversification. The fixed rule ignores the fact that the marginal value of an additional sample differs across products, test environments, and due-date settings. As a result, it may allocate testing effort to products for which additional samples provide little scheduling benefit while failing to relieve bottlenecks for more critical projects.
