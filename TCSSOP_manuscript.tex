...
The refrigerator industry operates under intense global competition, compressed innovation cycles, and increasingly strict market-specific performance, safety, and energy-efficiency requirements \citep{IEA2021,IEC60335-1,IEC60335-2-24,IEC62552}. Since products cannot proceed to certification, production, or commercialization before completing design approval tests, R\&D testing is a strategic operational bottleneck. Delays in testing postpone market entry and reduce responsiveness, while inefficient chamber use lowers the return on capital-intensive testing infrastructure \citep{OECD_MSTI2024,OECD_MSTI2025}. Launch-timing and quality-assurance decisions are also closely linked to commercial outcomes, particularly when time-to-market pressure and quality risk interact \citep{KimKoenigsbergOfek2022}.

From an operations-research perspective, refrigerator test planning combines resource-constrained multi-project scheduling, heterogeneous parallel resources, due-date pressure, and strict constraints. Practical scheduling systems increasingly require integrated treatment of precedence, resource heterogeneity, and eligibility restrictions \citep{HartmannBriskorn2022,GomezSanchezLallaRuizFernandezGilCastroVoss2023}. Similarly, unrelated parallel-machine scheduling with machine eligibility and tardiness objectives is known to be computationally challenging and often requires specialized heuristic methods \citep{UlagaDurasevicJakobovic2022,MaeckerShenMonch2023,AfzaliradRezaeian2016}.

This study is motivated by refrigerator R\&D testing operations at Vestel. The test portfolio includes mandatory and conditional tests selected according to target market, climatic class, energy regulation, electrical specification, and engineering decisions. Tests are executed in stages and require specific temperature, humidity, and sometimes voltage-compatible chamber conditions. Table~\ref{tab:test_attributes} summarizes the test catalog, and Figure~\ref{fig:chamber_structure} illustrates the chamber-station structure. Since each chamber contains multiple stations operating under a shared environment, chamber configuration and station-level scheduling are tightly coupled.
...
A key practical feature is the use of physical samples. In the industrial testing facility considered in this study, a fixed three-sample policy is commonly used because it is simple to implement. However, sample size is itself a scheduling decision rather than a fixed convention. Pull-down tests scale with the number of samples and increase upstream workload, whereas additional samples can enable downstream tests to run in parallel on different physical prototypes and reduce completion delays. Increasing sample count therefore does not guarantee lower tardiness: the relationship is non-monotone and depends on due dates, chamber eligibility, environmental bottlenecks, and stage-based precedence logic. A natural question is whether performance can be improved simply by raising the uniform sample level, or whether project-level allocation under a given operational setting is what matters.

This setting differs from existing integrated planning streams. In order acceptance and scheduling, workload selection and sequencing are coordinated, but the processing workload is generally exogenous \citep{Slotnick2011,WangYe2019}. In reliability demonstration testing, sample size is optimized under statistical risk and confidence considerations, but detailed resource-constrained scheduling is usually not modeled \citep{Fernandez2022}. The problem studied here requires both endogenous workload generation through sample decisions and feasible scheduling on constrained heterogeneous test chambers.

We define this problem as the Test Chamber Scheduling with Sample Optimization Problem (TCSSOP). The objective is to minimize total tardiness, with total sample count used as a secondary lexicographic criterion: among equal-tardiness solutions, fewer samples are preferred. TCSSOP jointly determines chamber-environment assignments, project-level sample-size decisions, and feasible schedules under temperature, humidity, voltage, station-capacity, sample-availability, and stage-based precedence constraints.

To solve TCSSOP, we propose an iterative framework. For a given sample vector, chamber environments are first generated through workload-balancing logic and further refined by local search. Based on these resulting chamber settings, sample sizes are improved through bounded local search, with each candidate evaluated by an EDD-based constructive scheduler that explicitly validates feasibility. Since sample size decisions change the workload distribution across environments, chamber assignments are re-optimized iteratively. When local improvement stagnates, a VNS diversification mechanism perturbs sample sizes and re-optimizes the resulting workload structure.

This study makes four main contributions. First, it benchmarks fixed uniform sample policies against endogenous sample-size optimization and quantifies their effect on total tardiness. Second, it includes an equal-budget control experiment in which a uniform six-sample policy uses the same average prototype consumption as the optimized solution, thereby isolating the effect of project-level allocation from total sample quantity. Third, it develops an iterative framework that coordinates sample-size decisions, chamber-environment assignment, and feasible scheduling. Fourth, it introduces a VNS diversification strategy that perturbs workload-defining sample size decisions rather than only sequencing or assignment decisions.

The remainder of the paper is organized as follows. Section~\ref{sec:literature_review} reviews related literature. Section~\ref{sec:problem_definition} defines the problem and notation. Section~\ref{sec:solution_methodology} presents the solution methodology. Section~\ref{sec:computational_study} reports computational results, and Section~\ref{sec:conclusion} concludes the study.
...
A first relevant stream is unrelated parallel-machine and resource-constrained scheduling. In unrelated parallel-machine environments, machine eligibility, delivery-time constraints, and tardiness-oriented objectives significantly increase problem difficulty. Recent studies show that local-search and VNS-based methods can perform effectively in such settings, especially when neighborhood structures and decoding procedures are carefully designed \citep{UlagaDurasevicJakobovic2022,MaeckerShenMonch2023,AfzaliradRezaeian2016}. More broadly, resource-constrained multi-project scheduling provides a natural reference for systems in which multiple projects compete for limited resources under precedence and feasibility constraints. Recent surveys emphasize the importance of incorporating resource heterogeneity, precedence logic, and practical constraints into scheduling models \citep{HartmannBriskorn2022,GomezSanchezLallaRuizFernandezGilCastroVoss2023}. Priority-rule studies further show that due-date-oriented dispatching rules, including earliest-due-date variants, remain competitive baselines under multi-project resource contention \citep{WangHeKerkhoveVanhoucke2017}. Closely related laboratory and burn-in settings also motivate modeling shared environmental resources as non-identical parallel batch processors with tardiness objectives \citep{HulettDamodaranAmouie2017}.
...
Test requirements are product-dependent and are determined by the target market, climatic class, energy regulation, electrical specification, and optional engineering decisions. Therefore, each product may require a different subset of tests and a different set of chamber environments.
